...
export interface UserSettings { name: string; theme: Theme; currency: "GBP"; timezone: string; firstDayOfWeek: 0 | 1; waterTargetMl: number; sleepTargetHours: number; socialLimitMinutes: number; fitnessWeeklyTarget: number; readingDailyMinutes: number; businessWeeklyHours: number; outreachWeeklyTarget: number; learningWeeklySessions: number; onboardingComplete: boolean; lastBackupAt?: string; }
...
export interface Project { id: string; name: string; clientId?: string; description: string; stage: string; deadline: string; progress: number; priority: string; techStack: string; repositoryUrl: string; liveUrl: string; price: number; paid: number; }
export interface ExpenseEntry { id: string; amount: number; date: string; category: string; description: string; scope: "business" | "personal"; }
export interface IncomeEntry { id: string; amount: number; date: string; source: string; clientId?: string; projectId?: string; scope: "business" | "personal"; description: string; }
export interface Goal { id: string; name: string; label: string; target: number; targetDate?: string; accent: string; }
export interface GoalContribution { id: string; goalId: string; amount: number; date: string; note: string; }
